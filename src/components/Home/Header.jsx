...
useEffect(()=>{
       ...
       
       {
        ...
        <div className={styles["userCredentialCont"]}>
        {
          myCont.isLoggedIn ? (
            ...
            <Avatar alt="Remy Sharp" src="https://thumbs.dreamstime.com/b/customer-support-service-agent-headset-flat-vector-icon-design-designs-153069456.jpg" className={styles['avatar']} />
            </Link>
          ): (<Link to={'/signin'}><Button className={`${styles['signin-btn']}`}>Sign In</Button></Link>)
        }
             
          

            <Link to={'/'} className={styles["wallet"]}><span><FaRupeeSign/>1800.00</span></Link>
            <Link to={'/cart'}><span className={styles["addtoCart"]}><HiOutlineShoppingBag/></span></Link>
        </div>
      </nav>

      <div ref={subNavRef} onMouseLeave={handleMouseLeave} className={` ${styles["categoriesCont"]} ${
          isSticky ? styles["sticky"] : ""
        }`}>

    <div  className={`d-flex flex-column ${styles['allCategoriesMainCont']}`}>
        <div className={styles['allCategories']} onClick={handleAllCategoriesDropDown}>
          <span><IoIosMenu/></span>
  ...
  <div className={`${styles['slider-category-cont']} ${isSliderAllCategory==='Electronics' ? styles['slider-active']:""} `} style={{visibility:isSliderAllCategory?'visible':'hidden'}}>

  <ul>
    <li><Link><Button>Laptops</Button></Link></li>
    <li><Link><Button>Smart Watches</Button></Link></li>
    <li><Link><Button>Cameras</Button></Link></li>
  </ul>

</div>
         ):(["Groceries", "Beauty", "Wellness","Sports","Jewellery"].includes(isSliderAllCategory) ?(<></>):
         (<div className={`${styles['slider-category-cont']} ${isSliderAllCategory==='Fashion'||isSliderAllCategory==='Bags'||isSliderAllCategory==='Footwear'?styles['slider-active']:""} `}  style={{visibility:isSliderAllCategory?'visible':'hidden'}}>

          <ul>
            <li><Link><Button>Men</Button></Link></li>
            <li><Link><Button>Women</Button></Link></li>
          </ul>
